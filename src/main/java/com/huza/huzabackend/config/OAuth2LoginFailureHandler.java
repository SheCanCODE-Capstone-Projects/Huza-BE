...
public class OAuth2LoginFailureHandler extends SimpleUrlAuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest request,
        ...
        String message = missingAuthorizationResponse
                ? "Invalid OAuth2 callback. Start sign-in at /login or open the authorizationUrl returned by "
                    + "/api/auth/oauth2/google; do not open /login/code/google directly."
                : "user_not_found".equals(errorCode)
                    ? "User not found. Please register before logging in with Google."
                    : "Google authentication failed. Verify GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and the "
                    + "authorized redirect URI (http://localhost:8016/login/code/google).";
